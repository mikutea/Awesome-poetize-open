...
export default {
  ...
  methods: {
     ...
     */
    initChannels() {
      this.channelStatus = Array(this.channels).fill(null).map(() => ({
    ...
    sendDanmaku(now) {
      ...
      const isAlreadyShowing = this.renderList.some(item => item.id === danmaku.id)
      if (isAlreadyShowing) {
    ...
    findAvailableChannel(now) {
      ...
      for (let i = 0; i < this.channels; i++) {
        ...
        const currentLeft = this.containerWidth - (this.containerWidth + estimatedWidth) * progress
    ...
    getAvatarUrl(item) {
      ...
      if (item.avatar && item.avatar.trim() && item.avatar !== this.defaultAvatar) {
        return item.avatar
      }
      // 2. 否则使用随机头像
    ...
    getDanmakuStyle(item) {
      ...
      // 计算位置
      const left = this.containerWidth - (this.containerWidth + estimatedWidth) * progress
    ...
    handleDanmakuMouseEnter(item) {
      if (this.pauseOnHover) {
        // 标记这条弹幕为暂停状态
        this.$set(item, 'isPaused', true)
        // 记录暂停时的已运行时间
        this.$set(item, 'pausedElapsed', Date.now() - item.startTime)
        this.$set(item, 'pauseStartTime', Date.now())
      }
    },
    ...
    handleDanmakuMouseLeave(item) {
      if (this.pauseOnHover && item.isPaused) {
        ...
        // 清除暂停状态
        this.$set(item, 'isPaused', false)
        this.$set(item, 'pausedElapsed', undefined)
        this.$set(item, 'pauseStartTime', undefined)
      }
    },
